...
- **複数ゲームへの自動適応**: JSONベースのデータ駆動アーキテクチャを採用。固有modフォルダを検索して対応するゲーム設定ファイルを動的にロードします。
- **調停システム**: 他の看板システム(mod)との調停を行います。
...
- **VoxeLibre / MineClone2 環境時 (`mineclone2.json`)**: 
  オークから歪んだ木まで、**全11種類のバリエーション**へ看板を拡張します。元の古い看板のレシピのみを自動で消去し、クリエイティブモードでのみの利用に制限します。
  サバイバルモードでは新しい看板システムへと置き換えます。
- **Mineclonia 環境時 (`mineclonia.json`)**: 
  サンプルとして「鉄」の看板が追加されます。
...
- **最低環境**: **Luanti v5.15.2 以上** (エンジンの最適化および、重要なセキュリティ対策パッチが適用されているため推奨)。
- **推奨環境**: **Luanti v5.17.0 以上** (重要なセキュリティ対策パッチが適用されているため推奨)。
